...
hatchling picks it up via ``packages = ["src/sim_plugin_gmsh"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_gmsh-*.whl"))
    ...
    required = {
        "sim_plugin_gmsh/__init__.py",
        "sim_plugin_gmsh/driver.py",
        "sim_plugin_gmsh/_skills/gmsh/SKILL.md",
    }
